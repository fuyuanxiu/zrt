...
@Service(value = "GlueInputService")
@Transactional(propagation = Propagation.REQUIRED)
public class GlueInputImpl implements GlueInputService {
    ...
    public ApiResponseResult afterDevice(String deviceNo) throws Exception {
        ...
        if(resultList.size() > 0){
          ...
          List arr = new ArrayList<>();
            ...
            //map.put("list", arr);
            map.put("list", this.getAfterDeviceListRf("10000", "1000", deviceNo, 100, 1).get(0));
            if(list_strs.length >0){
                ...
                if(last.length < 4){
                    ...
                    map.put("classes", "");
                    map.put("remark", "");
                    return ApiResponseResult.success("返回数据格式不正确，无法显示正确数据！").data(map);
                }
    ...
    public List<Object> getAfterDeviceListRf(String factory,String company,String in_str,int page,int size) {
        List<Object> resultList = (List<Object>) jdbcTemplate.execute(new CallableStatementCreator() {
            @Override
            public CallableStatement createCallableStatement(Connection con) throws SQLException {
                String storedProc = "{call App_Zhijiao_Listcurr(?,?,?,?,?,?,?,?)}";// 调用的sql
    ...
    public ApiResponseResult afterQty(String param) throws Exception{
        ...
        List<String> resultList = this.doPrc(param,"Prc_Zhijiang_Put_NTsk");
        System.out.println(resultList);
        if(resultList.size() > 0){
            String s = resultList.get(0).substring(0);
            String[] strs = s.split("\\[");
    ...
    public List<Object> getListRf(String factory,String company,String in_str,int page,int size) {
        List<Object> resultList = (List<Object>) jdbcTemplate.execute(new CallableStatementCreator() {
            @Override
            public CallableStatement createCallableStatement(Connection con) throws SQLException {
                String storedProc = "{call App_Zhijiao_Listhead(?,?,?,?,?,?,?,?)}";// 调用的sql
                CallableStatement cs = con.prepareCall(storedProc);
                cs.setString(1, factory);
                cs.setString(2, company);
                cs.setString(3, in_str);
            ...
            public Object doInCallableStatement(CallableStatement cs) throws SQLException, DataAccessException {
                ...
                if(cs.getString(6).toString().endsWith("0")){
                    ...
                    while(rs.next()){
                        Map m = new HashMap();
                        m.put("ID", getEmpty(rs.getString("ID")));
                        m.put("LOT_NO", getEmpty(rs.getString("LOT_NO")));
                        m.put("FPEIFANG", getEmpty(rs.getString("FPEIFANG")));
	...
	public ApiResponseResult getDetailList(String pid) throws Exception {
		// TODO Auto-generated method stub
		 List<Object> a  = this.getDetailListRf("10000", "1000",pid);
	        if(!a.get(0).toString().equals("0")){
	            return ApiResponseResult.failure(a.get(1).toString());
	        }
	...
	public List<Object> getDetailListRf(String factory,String company,String pid) {
        List<Object> resultList = (List<Object>) jdbcTemplate.execute(new CallableStatementCreator() {
            @Override
            public CallableStatement createCallableStatement(Connection con) throws SQLException {
                String storedProc = "{call App_Zhijiao_Listdetail(?,?,?,?,?,?)}";// 调用的sql
                CallableStatement cs = con.prepareCall(storedProc);
                cs.setString(1, factory);
                cs.setString(2, company);
                cs.setString(3, pid);
